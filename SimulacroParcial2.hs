...
porcentajeTotal :: Integer -> [Integer] -> Float
porcentajeTotal _ [] = 0
porcentajeTotal cantTotalVotos votos = division (primero votos*100) cantTotalVotos + porcentajeTotal cantTotalVotos (tail votos)

porcentajeDeVotosAfirmativos :: [(String,String)] -> [Integer] -> Integer -> Float
porcentajeDeVotosAfirmativos formulas votos cantTotalVotos = porcentajeTotal cantTotalVotos votos
...
pertenece a [] = False
pertenece a (x:xs)| a == fst x || a == snd x = True
                  | otherwise = pertenece a xs 

hayTuplasConIgualValor :: Eq a => [(a, a)] -> Bool
hayTuplasConIgualValor [(a,b)] = False
hayTuplasConIgualValor ((a,b):xs) | a == b = True
                                  | otherwise = hayTuplasConIgualValor xs

hayTuplasIguales :: (Eq a) => [(a,a)] -> Bool
hayTuplasIguales [(a,b)] | a == b = True
                         | otherwise = False
hayTuplasIguales ((a,b):xs)| a == b = True
                           | otherwise = hayTuplasIguales xs

eliminarNombresRepetidos :: (Eq a) => [(a,a)] -> [(a,a)]
eliminarNombresRepetidos [x] = [x]
eliminarNombresRepetidos (x:xs) | pertenece (fst x) xs = eliminarNombresRepetidos xs
                                | pertenece (snd x) xs = eliminarNombresRepetidos xs
                                | otherwise = x:eliminarNombresRepetidos xs

hayNombresRepetidos :: (Eq a) => [(a,a)] -> Bool
hayNombresRepetidos [x] = False
hayNombresRepetidos (x:xs) | pertenece (fst x) xs = True
                           | pertenece (snd x) xs = True
                           | otherwise = hayNombresRepetidos xs

formulasInvalidas :: [(String,String)] -> Bool
formulasInvalidas [] = False
formulasInvalidas ((a,b):xs)| hayTuplasConIgualValor ((a,b):xs) = True
